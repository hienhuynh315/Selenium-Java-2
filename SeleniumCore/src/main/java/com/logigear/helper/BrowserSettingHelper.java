...
public class BrowserSettingHelper {
	public static DriverProperty getDriverProperty(String file,
		...
		if (section == null) {
			throw new Exception(String.format("Cannot find '%s' in file '%s'",
					sectionName, file));
		}
	...
	private static RunningMode converStringToRunningMode(String mode)
			throws Exception {
		try {
			...
			throw new Exception(
					String.format(
							"Don't allow the '%s'. Please use %s for your configuration",
							mode, RunningMode.asString()));
		}
	}

	private static DriverType converStringToDriverType(String type)
			throws Exception {
		try {
